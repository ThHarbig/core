...
import mayday.core.io.dataset.SimpleSnapshot.Snapshot_v2_2;
...
public class Export_2_2 extends AbstractPlugin implements DatasetFileExportPlugin {
	
	@SuppressWarnings("unchecked")
	public PluginInfo register() throws PluginManagerException {
		PluginInfo pli = new PluginInfo(
				(Class)this.getClass(),
				"PAS.zippedsnapshot.write.2_2",
				new String[0],
				Constants.MC_DATASET_EXPORT,
				new HashMap<String, Object>(),
				"Florian Battke",
				"battke@informatik.uni-tuebingen.de",
				"Writes a zipped Mayday 2.5 snapshot of a Dataset.",
				"Zipped Snapshot Export (Mayday 2.09)"
		);
...
//		pli.getProperties().put(GUDEConstants.EXPORTER_DESCRIPTION,"Old Snapshot Format (until Mayday 2.9)");
		pli.getProperties().put(GUDEConstants.TYPE_DESCRIPTION,"Old Snapshot Format (until Mayday 2.09)");		
		return pli;
	}
		...
		AbstractTask at = new AbstractTask("Writing Snapshot") {
			...
			protected void doWork() throws Exception {
				...
				for (DataSet ds : datasets) {
					...
					Snapshot snap = new Snapshot_v2_2();
					snap.setDataSet(ds);
