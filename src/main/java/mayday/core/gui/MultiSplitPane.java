/*
 * $Id: MultiSplitPane.java,v 1.1 2011/05/26 08:26:32 battke Exp $
 *
...
import mayday.core.gui.MultiSplitLayout.Divider;
import mayday.core.gui.MultiSplitLayout.Node;
...
public class MultiSplitPane extends JPanel {
    ...
    public class AlwaysDividerPainter extends DividerPainter {
    	public void paint(Graphics g, Divider divider) {
    		Graphics2D g2d = (Graphics2D)g;
    		g2d.setColor(Color.gray);
    		Rectangle2D r = divider.getBounds();
    		if (divider.isVertical()) {
    			double width = r.getMaxY()-r.getMinY(); 
    			if (width>20)
    				width = 20;    			
    			r = new Rectangle2D.Double(
    					r.getCenterX(),
    					r.getCenterY()-width/2,
    					1,
    					width);
    			g2d.fill(r);
    			g2d.fill(new Rectangle2D.Double(r.getCenterX()-3d, r.getY()+5d, 1d, width-10d));
    			g2d.fill(new Rectangle2D.Double(r.getCenterX()+1d, r.getY()+5d, 1d, width-10d));
    		} else {
    			double width = r.getMaxX()-r.getMinX(); 
    			if (width>20)
    				width = 20;    		
    			r = new Rectangle2D.Double(
    					r.getCenterX()-width/2,
    					r.getCenterY(),
    					width,
    					1);
    			g2d.fill(r);
    			g2d.fill(new Rectangle2D.Double(r.getX()+5d, r.getCenterY()-3d, width-10d, 1d));
    			g2d.fill(new Rectangle2D.Double(r.getX()+5d, r.getCenterY()+1d, width-10d, 1d));
    		}
