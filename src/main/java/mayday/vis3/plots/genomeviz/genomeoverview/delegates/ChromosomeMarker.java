...
public abstract class ChromosomeMarker {
	...
	public static int tickToIndex(double tick, double min, double max, int n) {
		// PRETTIFY THE RANGE
		...
		double di = Math.abs(ticks[1]-ticks[0])/ticks[2]; // the tick step
		
		// round tick down to nearest position
		tick -= ticks[0];
		tick /= di;
	...
	public static TPU tickmark_power_unit(int position, double min, double max, int n) {
		return tickmark_power_unit(position, min, max, n, false, true);
	}
	
	public static TPU tickmark_power_unit(int position, double min, double max, int n, boolean lower, boolean upper) {
		...
		double di = Math.abs(ticks[1]-ticks[0])/ticks[2];

		double theTick = ticks[0]+position*di;

		// FIND THE CORRECT UNIT SIGN and the MULTIPLYING POWER
		...
		if(x>=1) //UUNITS
		{
			if(upper)
			{
				int nn = ((int)Math.floor(Math.log10(x))); //number of positions - 1;
				int i = nn / 3; //number of group, i.e. index of unit in UUNITS;
				if(i>5) i=5;
				thePower=Math.pow(10,i*-3);                
				theUnit = UUNITS[i];
			}else
			{
				thePower = 1.0;
		...
		}else //LUNITS
		{
			if(lower)
			{
				int nn = ((int)Math.floor(Math.log10(1/x))); //number of positions - 1;
				int i = nn / 3; //number of group, i.e. index of unit in UUNITS;
				if(i>5) i=5;
				thePower=Math.pow(10,i*3);                
				theUnit = LUNITS[i];
		...
		ret.power=thePower;
		ret.unit=theUnit;
		ret.tickCount=(int)(Math.abs(ticks[1]-ticks[0])/di);
		return ret;
	 ...
	 */
	private static double[] tickmarks(double min, double max, int n, int min_n)
	{
		/*
		 * defaults
		 */
	...
	private static double[] tickmarks_pretty0(
