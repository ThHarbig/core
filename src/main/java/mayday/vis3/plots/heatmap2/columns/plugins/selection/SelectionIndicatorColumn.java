...
public class SelectionIndicatorColumn extends AbstractInertColumn implements SettingChangeListener {
	...
	protected ColorSetting color1, color2;
	
	public SelectionIndicatorColumn(HeatmapStructure struct) {
		data = struct;
		...
		color1 = new ColorSetting("Selected Color",null, Color.red);
		color2 = new ColorSetting("Normal Color",null, Color.white);

		setting = new HierarchicalSetting("Divider Column").addSetting(wtype).addSetting(color1).addSetting(color2);
	...
	public void render(Graphics2D graphics, int row, int col, boolean isSelected) {		
		graphics.setColor(isSelected?color1.getColorValue():color2.getColorValue());
