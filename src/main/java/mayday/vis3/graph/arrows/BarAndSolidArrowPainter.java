...
public class BarAndSolidArrowPainter extends SolidArrowPainter
{

	@Override
	public Shape paintArrow(Point2D source, Point2D target, int l, double alpha) 
	{
		Shape arrow=super.paintArrow(source, target, l, alpha);
		Path2D res=new Path2D.Float();
		res.append(arrow,false);		
	
		double angle=angle(source, target);	
        double x3 = target.getX() + l * 1.4 * Math.cos(angle);
        double y3 = target.getY() + l * 1.4 * Math.sin(angle);
