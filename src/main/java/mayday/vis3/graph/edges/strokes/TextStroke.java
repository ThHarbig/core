...
public class TextStroke implements Stroke {
	...
	public TextStroke( String text, Font font, int position, int offset ) {
		this( text, font, false, false );
...
//				first = true;
                ...
                switch(position)
                {
                case BEGIN: next = nextAdvance+offset;
                	break;
                case CENTER: next = nextAdvance+(int)(pathLength/2.0);
            		break;
                case END: next = nextAdvance+(int)(pathLength)-offset;
                	next-=(float)glyphVector.getLogicalBounds().getWidth();
            		break;
            		default:  next = nextAdvance;
                }
				break;
			...
			case PathIterator.SEG_LINETO:
				...
				if ( distance >= next ) {
					...
					while ( currentChar < length /*&& distance >= next*/ ) {
						...
						float py = (float)p.getY()+lineOffset;
