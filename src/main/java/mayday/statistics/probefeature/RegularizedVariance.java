 ...
 *
 */
public class RegularizedVariance
...
implements ProbelistPlugin
{
	...
	public PluginInfo register() throws PluginManagerException {
		PluginInfo pli= new PluginInfo(
				(Class)this.getClass(),
				"PAS.statistics.regvar",
				new String[0], 
				Constants.MC_PROBELIST,
				(HashMap<String,Object>)null,
				"Florian Battke",
				"battke@informatik.uni-tuebingen.de",
				"Compute regularized expression value variance across all experiments. (var/min)",
				"Probe Regularized Variance");
		pli.addCategory(MC);
		return pli;
	}
	...
	public List<ProbeList> run( List<ProbeList> probeLists, MasterTable masterTable )
	{
		...
		MIGroup l_mioGroup = masterTable.getDataSet().getMIManager().newGroup("PAS.MIO.Double", 
				"Regularized variance","/Probe Statistic/"
				);
		...
		for ( int i = 0; i < l_uniqueProbes.size(); ++i )
		{
			...
			Double l_variance = new Double( l_probe.getVariance() / Math.abs(l_probe.getMinValue()) );

			if ( l_variance != null )
			{
				DoubleMIO l_mio = new DoubleMIO( l_variance );
