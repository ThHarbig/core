...
public class MMStrandArray implements Iterable<Strand> { //, DumpableStructure {
	
	protected final static Strand[] values = new Strand[]{
		Strand.MINUS, Strand.PLUS, Strand.BOTH, Strand.UNSPECIFIED
	};
	
	protected MMLongArray content;
	protected long len = 0;
	
	public MMStrandArray(int blocksize) {
		content = new MMLongArray((int)Math.ceil((double)blocksize/32d));
	}
	
	public MMStrandArray(MMStrandArray other) {
		content = new MMLongArray(other.content);
	}
