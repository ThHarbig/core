...
public class DArea extends JComponent implements DParent, Printable
{
	...
	public void setMaxY( double may ){
		...
		new IllegalArgumentException(
				"Maximal y-value axes intersects minmal rectangle."+min_rect.y+" "+min_rect.height+" "+may);
	...
	public void paint( Graphics g ){
		...
		// hide all topmost children for this call of paint->paintChildren
		for (Component c : getComponents())
			if (c instanceof TopMostChild)
				c.setVisible(false);
						
		super.paint( g );
		
		g = measures.configureGraphics(g);
        // set the font according to the component font
		g.setFont(getFont());

		
		if( grid.isVisible() && !grid_to_front ) paintGrid( g, measures );
		container.paint( g, measures );
		if( grid.isVisible() && grid_to_front ) paintGrid( g, measures );

		// paint all topmost children now
		paintTopMostChildren(g);
	}

	protected void paintTopMostChildren(Graphics g) {
		synchronized(getTreeLock()) {
		    for (Component comp : getComponents()) {
		    	if (comp instanceof TopMostChild) {
	...
	{
		return grid.hor_dist;
	}

	public double getGridY()
	{
		return grid.ver_dist;
	}

	public double getGridEmphX()
	{
		return grid.hor_emph;
	}

	public double getGridEmphY()
	{
		return grid.ver_emph;
	}
	...
	public void setGridEmphasize(double hor, double ver)
	{
		grid.hor_emph = hor;
		grid.ver_emph = ver;
		auto_grid = false;
	 ...
	 */
	private void paintGrid( Graphics g, DMeasures m ){
	 ...
	 */
	private void paintGrid(Graphics g, ScaledBorder sb, DMeasures m){
		...
		if( m.x_scale == null && m.y_scale == null ) grid.paint( g, m );
