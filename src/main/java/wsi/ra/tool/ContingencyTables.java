...
public class ContingencyTables
{
    ...
    public static double CramersV(double[][] matrix)
    {
        ...
        if ((min == 0) || StatisticUtils.eq(n, 0))
        {
            return 0;
        }
    ...
    public static double chiVal(double[][] matrix, boolean useYates)
    {
        ...
        for (row = 0; row < nrows; row++)
        {
            if (StatisticUtils.gr(rtotal[row], 0))
            {
                for (col = 0; col < ncols; col++)
                {
                    if (StatisticUtils.gr(ctotal[col], 0))
                    {
            ...
            if (StatisticUtils.gr(rtotal[row], 0))
            {
                for (col = 0; col < ncols; col++)
                {
                    if (StatisticUtils.gr(ctotal[col], 0))
                    {
                        ...
                        if (StatisticUtils.sm(expect, smallfreq))
                        {
                            if (StatisticUtils.sm(expect, 1))
                            {
                                return false;
                            }
                            else
    ...
    {
        double returnValue = 0;
        double trainSumForRow;
        double testSumForRow;
        double testSum = 0;
    ...
    public static double gainRatio(double[][] matrix)
    {
        ...
        if (StatisticUtils.eq(splitEnt, 0))
        {
            return 0;
        }
    ...
    public static double log2MultipleHypergeometric(double[][] matrix)
    {
        ...
        for (int i = 0; i < matrix.length; i++)
        {
            ...
            sum += Statistics.lnFactorial(sumForRow);
            total += sumForRow;
        }
        ...
        for (int j = 0; j < matrix[0].length; j++)
        {
            ...
            sum += Statistics.lnFactorial(sumForColumn);
        }
        ...
        for (int i = 0; i < matrix.length; i++)
        {
            for (int j = 0; j < matrix[i].length; j++)
            {
                sum -= Statistics.lnFactorial(matrix[i][j]);
            }
        }

        sum -= Statistics.lnFactorial(total);
    ...
    public static void main(String[] ops)
    {
        ...
        System.out.println(
            "Negative log2 of multiple hypergeometric probability: " +
    ...
    public static double symmetricalUncertainty(double[][] matrix)
    {
        ...
        double rowEntropy = 0;
        double entropyConditionedOnRows = 0;
        double infoGain = 0;
        ...
        if (StatisticUtils.eq(columnEntropy, 0) ||
                StatisticUtils.eq(rowEntropy, 0))
        {
            return 0;
        }
    ...
    public static double tauVal(double[][] matrix)
    {
        ...
        for (row = 0; row < nrows; row++)
        {
            ...
            for (col = 0; col < ncols; col++)
            {
                if (StatisticUtils.gr(matrix[row][col], max))
                {
                    max = matrix[row][col];
                }
        ...
        if (StatisticUtils.eq(n, 0))
        {
            return 0;
        }

        maxcol = ctotal[StatisticUtils.maxIndex(ctotal)];
    ...
    private static double chiCell(double freq, double expected, boolean yates)
    {
        ...
        if (StatisticUtils.smOrEq(expected, 0))
        {
            return 0;
        }
